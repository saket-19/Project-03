...
/**
 * ChatRoom List functionality controller.to perform Search and List operation.
 * 
 * @author saket
 *
 */
@WebServlet(name = "ChatRoomListCtl", urlPatterns = { "/ctl/ChatRoomListCtl" })
public class ChatRoomListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(ChatRoomListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("ChatRoomListCtl doGet Start");
		List list;
		List next;
		int pageNo = 1;
		...
		log.debug("ChatRoomListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("ChatRoomListCtl doPost Start");
		List list = null;
		List next = null;
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					ChatRoomDTO deletedto = new ChatRoomDTO();
					for (String id : ids) {
			...
			dto = (ChatRoomDTO) populateDTO(request);
		...
		log.debug("ChatRoomListCtl doPost End");
	}

	@Override
	protected String getView() {
		return ORSView.CHAT_ROOM_LIST_VIEW;
	}
}