...
/**
 * Gender List functionality controller.to perform Search and List operation.
 * 
 * @author saket
 *
 */
...
public class GenderListCtl extends BaseCtl {
	...
	private static Logger log = Logger.getLogger(GenderListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("GenderListCtl doGet Start");
		List list;
		List next;
		int pageNo = 1;
		...
		log.debug("GenderListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("GenderListCtl doPost Start");
		List list = null;
		List next = null;
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					GenderDTO deletedto = new GenderDTO();
					for (String id : ids) {
			...
			dto = (GenderDTO) populateDTO(request);
		...
		log.debug("GenderListCtl doPost End");
	}
