...
/**
 * MobileVerification List functionality controller.to perform Search and List operation.
 * 
 * @author saket
 *
 */
@WebServlet(name = "MobileVerificationListCtl", urlPatterns = { "/ctl/MobileVerificationListCtl" })
public class MobileVerificationListCtl extends BaseCtl {

	private static final long serialVersionUID = 1L;
	private static Logger log = Logger.getLogger(MobileVerificationListCtl.class);
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		log.debug("MobileVerificationListCtl doGet Start");
		List list;
		List next;
		int pageNo = 1;
		...
		log.debug("MobileVerificationListCtl doGet End");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		log.debug("MobileVerificationListCtl doPost Start");
		List list = null;
		List next = null;
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					MobileVerificationDTO deletedto = new MobileVerificationDTO();
					for (String id : ids) {
			...
			dto = (MobileVerificationDTO) populateDTO(request);
		...
		log.debug("MobileVerificationListCtl doPost End");
	}

	@Override
	protected String getView() {
		return ORSView.MOBILE_VERIFICATION_LIST_VIEW;
	}
}