...
@WebServlet(urlPatterns={"/ctl/LocationListCtl"})
public class LocationListCtl extends BaseCtl{
	...
	protected BaseDTO populateDTO(HttpServletRequest request) {
		...
		dto.setCity(DataUtility.getString(request.getParameter("state")));
		dto.setCity(DataUtility.getString(request.getParameter("country")));
		dto.setCity(DataUtility.getString(request.getParameter("status")));
		...
		try {
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					LocationDTO deletebean = new LocationDTO();
					for (String id : ids) {
